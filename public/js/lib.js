...
function lib_listarCadastro(res,obj){
    ...
    try {
        var name = obj.attr('name'),loc_name=name.replaceAll('nome_designado','id_designado');
        if(loc_name&&res.id){
            $('[name="'+loc_name+'"]').val(res.id);
        }
...
function lib_autocomplete(obs){
    var urlAuto = obs.attr('url_autocomplete');
...
function publicadores_autocomplete(obs){
    var urlAuto = obs.attr('url_autocomplete');
    // var data_selector = obs.data('selector'),d=decodeArray(data_selector);
    // try {
    //     if(typeof d.janela != 'undefined'){
...
function add_designation(dt){
    var sl_el='ul-'+dt,row = $('#'+sl_el+' li').length,ultimo=document.getElementById(sl_el).querySelector('li:last-child'),k=ultimo.getAttribute('data-key');
    let u_clone = ultimo.cloneNode(true);
    row=new Number(row);
    row = row+1;
    if(row==k){
        row = row+1;
    }
    var id_li_an = 'li-'+dt+'-'+k;
    var id_li_at = 'li-'+dt+'-'+row;
    u_clone.setAttribute('id',id_li_at);
    u_clone.setAttribute('data-key',row);
    var inputs = u_clone.querySelectorAll('input');
    console.log('row:'+ row+ ' k: '+k);
    inputs.forEach( el => {
        el.value = '';
        var nAt = el.getAttribute('name'),idAt = el.getAttribute('id');
        var oldname = '['+k+']';
        var newname = '['+row+']';
        var nCha = nAt.replaceAll(oldname,newname);
        el.setAttribute('name',nCha);
        if(idAt){
            var idCha = idAt.replaceAll(oldname,newname);
            el.setAttribute('id',idCha);
        }
        console.log(nAt);
        // el.innerHTML = "foo";
    });
    console.log('id_li_an '+id_li_an+' id_li_at='+id_li_at);
    console.log(u_clone);
    document.getElementById(sl_el).append(u_clone);
}
function add_designation2(data,sessao){
    if(data && sessao){
        ...
        try {
           var id = seloc.querySelectorAll('li').length;
        } catch (er) {
            var id = 0;
        }
        var atr_li=tem.getAttribute('id');
        atr_li = atr_li.replaceAll('inicio_0', sessao+'_'+id);
...
function select_sessao(obj){
    var classe = obj.querySelector('option[selected=""]');
    console.log(classe);
}
function remove_designacao(id){
    if(!window.confirm('DESEJA MESMO REMOVER A DESIGANÇÃO?')){
        return;
    }
    getAjax({
        url:'/ajax/desigancao-remove',
        type: 'POST',
        dataType: 'json',
        csrf: true,
        data:{
